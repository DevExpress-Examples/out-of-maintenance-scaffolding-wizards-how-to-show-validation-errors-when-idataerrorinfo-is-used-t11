...
Namespace Scaffolding.ValidationErrors.Common.ViewModel
	Public MustInherit Partial Class CollectionViewModel(Of TEntity As Class, TPrimaryKey, TUnitOfWork As IUnitOfWork)
		Inherits CollectionViewModelBase(Of TEntity, TPrimaryKey, TUnitOfWork)
		Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IRepository(Of TEntity, TPrimaryKey)), Optional ByVal filterExpression As Expression(Of Func(Of TEntity, Boolean)) = Nothing)
	...
	Public MustInherit Class CollectionViewModelBase(Of TEntity As Class, TPrimaryKey, TUnitOfWork As IUnitOfWork)
		...
		Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IRepository(Of TEntity, TPrimaryKey)), ByVal filterExpression As Expression(Of Func(Of TEntity, Boolean)))
			MyBase.New(unitOfWorkFactory, getRepositoryFunc, filterExpression)
		End Sub
