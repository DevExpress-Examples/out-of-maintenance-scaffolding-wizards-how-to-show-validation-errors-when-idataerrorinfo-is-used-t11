...
Namespace Scaffolding.ValidationErrors.Common
	''' <summary>
	...
	Partial Public Class ReadOnlyCollectionViewModel(Of TEntity As Class, TUnitOfWork As IUnitOfWork)
		...
		''' <param name="getRepositoryFunc">A function that returns a repository representing entities of the given type.</param>
		''' <param name="projection">An optional parameter that provides a LINQ function used to customize a query for entities. The parameter, for example, can be used for sorting data.</param>
		Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IReadOnlyRepository(Of TEntity)), Optional ByVal projection As Func(Of IRepositoryQuery(Of TEntity), IQueryable(Of TEntity)) = Nothing, Optional ByVal unitOfWorkPolicy As UnitOfWorkPolicy = UnitOfWorkPolicy.Individual)
	...
	Partial Public Class ReadOnlyCollectionViewModel(Of TEntity As Class, TProjection As Class, TUnitOfWork As IUnitOfWork)
		...
		Public Shared Function CreateReadOnlyProjectionCollectionViewModel(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IReadOnlyRepository(Of TEntity)), ByVal projection As Func(Of IRepositoryQuery(Of TEntity), IQueryable(Of TProjection)), Optional ByVal unitOfWorkPolicy As UnitOfWorkPolicy = UnitOfWorkPolicy.Individual) As ReadOnlyCollectionViewModel(Of TEntity, TProjection, TUnitOfWork)
			Return ViewModelSource.Create(Function() New ReadOnlyCollectionViewModel(Of TEntity, TProjection, TUnitOfWork)(unitOfWorkFactory, getRepositoryFunc, projection, unitOfWorkPolicy))
		End Function
		...
		''' <param name="getRepositoryFunc">A function that returns the repository representing entities of a given type.</param>
		''' <param name="projection">A LINQ function used to customize a query for entities. The parameter, for example, can be used for sorting data and/or for projecting data to a custom type that does not match the repository entity type.</param>
		Protected Sub New(ByVal unitOfWorkFactory As IUnitOfWorkFactory(Of TUnitOfWork), ByVal getRepositoryFunc As Func(Of TUnitOfWork, IReadOnlyRepository(Of TEntity)), ByVal projection As Func(Of IRepositoryQuery(Of TEntity), IQueryable(Of TProjection)), Optional ByVal unitOfWorkPolicy As UnitOfWorkPolicy = UnitOfWorkPolicy.Individual)
			MyBase.New(unitOfWorkFactory, getRepositoryFunc, projection, unitOfWorkPolicy)
		End Sub
	End Class
End Namespace